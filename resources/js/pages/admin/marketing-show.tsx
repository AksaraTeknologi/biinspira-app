...
export default function MarketingShow({ data }: AdPlanProps) {
    ...
    // normalize platforms into an array whether the API returns a single object or an array
    const platformList: PlatformData[] = data?.platforms
        ? Array.isArray(data.platforms)
            ? data.platforms
            : [data.platforms]
        : [];

    // normalize results and evaluations to arrays
    ...
    // separate tab states so Plan tabs and Result tabs don't interfere
    const [planTab, setPlanTab] = useState<string>(() => {
        return platformList.length
            ? getPlatformKey(platformList[0].platform_name ?? platformList[0].name ?? undefined, 0)
            : 'platforms';
    });
