...
import {
    Bar, ResponsiveContainer, XAxis, Tooltip, Line,
    CartesianGrid, YAxis, ComposedChart,
} from "recharts";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import {
    ArrowDownRight, ArrowUpRight, Users, X,
    BarChart2, TrendingUp, ChevronDown, ChevronUp, ChevronRight
} from "lucide-react";
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: "Dashboard", href: "/user/marketing/dashboard" },
...
function CustomTooltip({ active, payload }: any) {
    ...
    return (
        <div className="p-2 rounded-md shadow-md border bg-background text-sm">
            <p className="font-semibold mb-1">{d.label}</p>
            <p className="text-xs text-gray-400">{d.user_name}</p>
            <p className="mt-1">Jumlah Peserta: <strong>{d.audience.toLocaleString("id-ID")}</strong></p>
            {!d.has_result && <p className="text-xs text-gray-400 mt-1">Belum ada hasil</p>}
        </div>
    );
}
            ...
            <CardContent className="pt-4 pb-3">
                <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${colorClass}`}>
                            {rank}
                        </div>
                        <div className="min-w-0">
                            <p className="font-semibold text-sm truncate">{userData.user_name}</p>
                        ...
                        {userData.events.map((ev) => (
                            <div key={ev.event_id}>
                                <div className="flex items-center justify-between mb-1.5">
                                    <p className="text-xs font-semibold truncate max-w-[60%]" title={ev.event_name}>{ev.event_name}</p>
                                    <span className="text-xs text-gray-500 font-medium">{ev.total.toLocaleString("id-ID")} peserta</span>
                                </div>
...
export default function AudienceChartPage() {
    ...
    const eventColorMap = useMemo(() => {
        ...
        events.forEach((e, i) => { map[e.id] = EVENT_COLORS[i % EVENT_COLORS.length]; });
        return map;
    }, [events]);
    ...
    const filteredUserData = useMemo(() => {
        ...
        })).filter(u => u.events.length > 0).sort((a, b) => b.total - a.total);
    }, [selectedEventIds, audiencePerUser]);

    // Event milik user yang sedang aktif di panel slide
    const activeUserData = useMemo(() =>
        audiencePerUser.find(u => u.user_id === activeUserId) ?? null,
        [activeUserId, audiencePerUser]
    );
    ...
    const defaultIncrease = filteredData.length > 1
        ? parseFloat((((filteredData[filteredData.length - 1].audience - filteredData[filteredData.length - 2].audience) /
            (filteredData[filteredData.length - 2].audience || 1)) * 100).toFixed(2))
        : 0;

    const selectedBatch = selectedIndex !== null && selectedIndex > 0
        ? { current: filteredData[selectedIndex], previous: filteredData[selectedIndex - 1] }
        : null;

    const percentageIncrease = selectedBatch
        ? parseFloat((((selectedBatch.current.audience - selectedBatch.previous.audience) /
            (selectedBatch.previous.audience || 1)) * 100).toFixed(2))
        : defaultIncrease;

    const currentLabel = selectedBatch
        ? selectedBatch.current.label
        : (filteredData[filteredData.length - 1]?.label ?? "");
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <Card>
                        <CardContent className="pt-5 flex items-center gap-3">
                            <div className="bg-blue-100 rounded-full p-2"><Users className="h-5 w-5 text-blue-600" /></div>
                            <div>
                                <p className="text-xs text-gray-400">Total Peserta</p>
                    ...
                    <Card>
                        <CardContent className="pt-5 flex items-center gap-3">
                            <div className="bg-green-100 rounded-full p-2"><TrendingUp className="h-5 w-5 text-green-600" /></div>
                            <div>
                                <p className="text-xs text-gray-400">Batch Terbanyak</p>
                                <p className="text-sm font-semibold truncate max-w-[160px]" title={topBatch?.label}>{topBatch?.label ?? "-"}</p>
                    ...
                    <Card>
                        <CardContent className="pt-5 flex items-center gap-3">
                            <div className="bg-purple-100 rounded-full p-2"><BarChart2 className="h-5 w-5 text-purple-600" /></div>
                            <div>
                ...
                <div>
                    <div className="mb-3">
                        <h2 className="text-base font-semibold">Peserta Per User</h2>
                        <p className="text-xs text-gray-400">Rincian jumlah peserta setiap user berdasarkan event dan batch</p>
                    </div>
                    {filteredUserData.length === 0 ? (
                        <div className="flex items-center justify-center h-24 text-gray-400 text-sm border rounded-lg border-dashed">
                            Belum ada data user
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {filteredUserData.map((userData, i) => (
                                <UserCard key={userData.user_id} userData={userData} colorClass={USER_COLORS[i % USER_COLORS.length]} rank={i + 1} />
                            ))}
                        </div>
                    )}
                </div>
                    ...
                    <div className={`flex flex-row gap-3 transition-all duration-300 ${activeUserId ? 'lg:col-span-2' : 'lg:col-span-1'}`}>
                        ...
                        <Card className="flex-1 h-fit">
                            ...
                            <CardContent className="flex flex-col gap-4">
                                ...
                                {/* Section: Filter Event */}
                                <div>
                                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Event</p>
                                    <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto pr-1">
                                        {events.map((event) => {
                                            const isSelected   = selectedEventIds.includes(event.id);
                                            const batchCount   = audiencePerBatch.filter(d => d.event_id === event.id).length;
                                            const totalPeserta = audiencePerBatch.filter(d => d.event_id === event.id).reduce((s, d) => s + d.audience, 0);
                                            return (
                                                <button
                                                    key={event.id}
                                                ...
                                                >
                                                    <div className="flex items-center gap-2">
                                                        <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: eventColorMap[event.id] }} />
                                                        <p className="font-medium truncate">{event.name}</p>
                                                    </div>
                                                    <p className="text-xs text-gray-400 mt-0.5 pl-4">
                                                        {batchCount} batch · {totalPeserta.toLocaleString("id-ID")} peserta
                                                    </p>
                                                </button>
                                            );
                                        })}
                                ...
                                {selectedEventIds.length > 0 && (
                                    <div className="flex flex-wrap gap-1 max-w-[300px] justify-end">
                                        {selectedEventIds.slice(0, 3).map(id => {
                                            ...
                                            return (
                                                <Badge key={id} variant="secondary" className="text-xs cursor-pointer" onClick={() => toggleEvent(id)}>
                                                    {ev?.name.slice(0, 10)}…<X className="h-2.5 w-2.5 ml-1" />
                                                </Badge>
                                            );
                                        })}
                                        {selectedEventIds.length > 3 && (
                                            <Badge variant="outline" className="text-xs">+{selectedEventIds.length - 3} lainnya</Badge>
                                        )}
                                    </div>
                                )}
                            </div>
                                ...
                                <div className="grid grid-cols-3">
                                    <div className="col-span-1 h-full flex flex-col py-4">
                                        ...
                                        <p className="text-gray-500 text-sm mb-9 flex flex-col">
                                            ...
                                            <span><strong>{currentLabel}</strong></span>
                                        </p>
                                        <div className="flex flex-col text-sm mt-auto gap-1">
                                            {events
                                                .filter(e => selectedEventIds.length === 0 || selectedEventIds.includes(e.id))
                                                .map(e => (
                                                    <div key={e.id} className="flex flex-row items-center gap-2">
                                                        <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: eventColorMap[e.id] }} />
                                                        <span className="truncate max-w-[100px]" title={e.name}>{e.name}</span>
                                                    </div>
                                                ))}
                                            <div className="flex flex-row items-center gap-2 mt-1">
                                                <span className="w-3 h-3 rounded-full bg-[#6BAED6]" />
                                        ...
                                        <div ref={containerRef} className="h-55 mt-auto">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <ComposedChart data={chartData} barCategoryGap="12%" barGap={8} style={{ minHeight: 50 }}>
                                                    ...
                                                    <XAxis dataKey="short_label" tickLine={false} axisLine={false} tick={{ fill: "#9ca3af", fontSize: 12 }} />
                                                    <YAxis yAxisId="left" hide />
                                                    <YAxis yAxisId="right" hide />
                                                    <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(0,0,0,0.05)" }} contentStyle={{ borderRadius: "8px", border: "none" }} />
                                                    <Bar dataKey="PendapatanHeight" yAxisId="left" radius={[50, 50, 50, 50]} barSize={barSize} filter="url(#barShadow)" shape={<CustomBar />} onClick={handleBarClick} />
