...
class AdResultPlatformController extends Controller
{
    public function resultForm($id_event, $id_platform, $id_ad_plan)
    {
        ...
        // check for adResult data by ad_plan_id
        $adResult = AdResult::where('ad_plan_id', $adPlan->id)->first();
        $adResultPlatform = $adResult
            ? AdResultPlatform::where('ad_result_id', $adResult->id)
            ->where('platform_id', $selectedPlatform->id)
            ->first()
            : null;
        $adMetric = $adResultPlatform
            ? AdMetric::where('ad_result_platform_id', $adResultPlatform->id)->first()
            : null;
        ...
        return Inertia::render('admin/markets/marketing/marketing-form2', [
            ...
            'adResultData' => [
                'adResult' => $adResult,
                'adResultPlatform' => $adResultPlatform,
                'adMetric' => $adMetric,
            ],
        ]);
    }
    public function storeOrUpdate(Request $request)
    {
        ...
        $adMetric = AdMetric::updateOrCreate(
        ...
        return redirect()
            ->route('admin.marketing.evaluation',["id" => $data["ad_plan_id"]])
            ->with('success', 'Data hasil iklan dan metrik berhasil disimpan atau diperbarui.');
    }
}
